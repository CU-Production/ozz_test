...
static void skeleton_data_loaded(const std::string& filename);
static void animation_data_loaded(const std::string& filename);
...
static void init(void) {
    ...
    {
        skeleton_data_loaded("data/ozz_skin_skeleton.ozz");
    }
    {
        animation_data_loaded("data/ozz_skin_animation.ozz");
    }
}
...
static void skeleton_data_loaded(const std::string& filename) {
    auto skeleton_data = readFile(filename);
    ...
    stream.Write(skeleton_data.data(), skeleton_data.size());
...
static void animation_data_loaded(const std::string& filename) {
    auto animation_data = readFile(filename);

    ozz::io::MemoryStream stream;
    stream.Write(animation_data.data(), animation_data.size());
    stream.Seek(0, ozz::io::Stream::kSet);
    ozz::io::IArchive archive(&stream);
...
sapp_desc sokol_main(int argc, char* argv[]) {
    ...
    desc.height = 600;
    desc.sample_count = 4;
    desc.window_title = "ozz_anim.cpp";
